...
Use this file (and the matching `AGENTS.md`) so new sessions do not need manual context about stack or folder layout.
...
Persistent Cursor rules for every chat live in `.cursor/rules/` (for example `botble-platform.mdc`). Keep this `CLAUDE.md` and `AGENTS.md` aligned when you change high-level project facts.
